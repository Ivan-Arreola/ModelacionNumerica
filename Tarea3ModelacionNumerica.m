...
% plot3(x,y,eta_ini)
% title('Condición inicial que da una perturbación inicial')
% stop
...
eta_n=eta_ini; eta_nm1=eta_n; %Los dos pasos para Eta
...
while t<T
    for iy=1:Ny
        ...
        uE_nm1(ix_s,iy_s)=uE_n(ix_s,iy_s)+Dt*f*(vN_n(ix_s,iy_s)+vN_n(ix_s,iy_s-1)+vN_n(ix_s+1,iy_s)+vN_n(ix_s+1,iy_s-1))/(4)-Dt*g*(eta_n(ix_s+1,iy_s)-eta_n(ix_s,iy_s))/(Dx);
...
plot3(x,y,H+eta_nm1);
title('Solución con un parámetro de coriolis f=50')